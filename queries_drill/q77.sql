-- start query 77 in stream 0 using template query77.tpl
 ...
 where ss_sold_date_sk = d_date_sk
       and cast(d_date as timestamp)  between cast('2000-08-02' as timestamp)
                  and (cast('2000-08-02' as timestamp) + interval  '30' day)
 ...
 select *
 from (
 select channel, cast(id as varchar) AS id, sales, sreturns, profit from  results
 union
 select channel, cast(NULL as varchar) AS id, sum(sales) as sales, sum(sreturns) as sreturns, sum(profit) as profit from  results group by channel
 union
 select cast(NULL as varchar) AS channel, cast(NULL as varchar) AS id, sum(sales) as sales, sum(sreturns) as sreturns, sum(profit) as profit from  results
) foo
order by channel, id
 limit 100;
