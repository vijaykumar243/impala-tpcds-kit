...
( select total_sum ,i_category ,i_class, g_category, g_class, 0 as lochierarchy from results
  union
  select sum(total_sum) as total_sum, i_category, cast(NULL as varchar) as i_class, 0 as g_category, 1 as g_class, 1 as lochierarchy from results group by i_category
  union
  select sum(total_sum) as total_sum, cast(NULL as varchar) as i_category, cast(NULL as varchar) as i_class, 1 as g_category, 1 as g_class, 2 as lochierarchy from results)
select 
