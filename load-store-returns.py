...
LOAD_FILE = "load_store_returns_tmp.sql"
...
def generate_queries(ss_sold_dates):
  num_part_per_query = int(ceil(0.5 * get_mem_limit())) * get_num_backends() * 10
  ...
  for partition_range in partition_ranges:
    query = """insert overwrite table store_returns
              partition(sr_returned_date) [shuffle]
              select
